...
\begin{document}
	...
	\textit{Замечание}. В гиперболической геометрии есть три типа пучков прямых: 1) прямые, пересекающиеся в одной точке; 2) параллельные прямые, пересекающиеся в одной точке; 3) прямые, перпендикулярные одной прямой. В каждом из этих случаев будем говорить, что прямые принадлежат одному пучку.
	...
	\begin{enumerate}[resume*]
		...
		\item
		\begin{enumerate}
			\item Доказать, что перпендикуляры к сторонам треугольника принадлежат одному пучку.
			\item Доказать, что высоты треугольника принадлежат одному пучку.
		\end{enumerate}
 
		\item Доказать формулу Герона:
		\[
			\sin\!\left(\frac{\Delta}{2}\right) = \frac{(e^s - 1)(e^{s-a} - 1)(e^{s-b} - 1)(e^{s-c} - 1)}{(e^s + 1)(e^{s-a} + 1)(e^{s-b} + 1)(e^{s-c} + 1)},
		\]
	...
	Модель Клейна получается из модели Пуанкаре в единичном круге следующим образом. Представим единичный круг как экватор сферы и спроецируем его из северного полюса сначала на экваториальную полуплоскость, а затем ортогонально спроецируем эту плоскость на единичный круг. В результате переходим сначала к единичной сфере, перпендикулярными экватору, а затем в единичный круг, перпендикулярными единичного круга.
 
	\begin{enumerate}[resume*]
		\item Доказать, что точки $Z$ и $W$ в модели Пуанкаре соответствуют точки $Z'$ и $W'$ в модели Клейна:
		\[
			w = \frac{2z}{|z|^2 + 1}, \quad \text{в модели Клейна.}
		\]
		...
		\item Доказать, что $\ln[A, B, Z, W] = 2\ln[A, B, Z', W']$.
 
		\item Доказать, что в модели Клейна прямые принадлежат одному пучку тогда и только тогда, когда прямые, содержащие их, принадлежат одному евклидову пучку.
 
		\item Доказать, что изображённые на рис. а) и б) построения соответственно позволяют построить серединный $M$ отрезка $AB$ и биссектрису угла между прямыми $a$ и $b$.
